...
FIELDNAMES = ["img_id", "img_h", "img_w", "num_boxes", "boxes", "features"]
    ...
    for (path, dir, files) in os.walk(img_root):
        for img_file in files:
            ...
            if ext == '.jpg':
                ...
                pathXid.append((file_path, idx))
    return pathXid

def generate_tsv(prototxt, weights, image_ids, outfile):
    img_list = json.load(open('img_list_train.json'))
    ...
    if '1053_Harry_Potter_and_the_philosophers_stone_00.50.38.499-00.50.40.303@0' in missing:
        print("YES")
    if len(missing) == 0:
...
def parse_args():
    ...
    parser.add_argument('--caffemodel', type=str, default='/workspace/features/resnet101_faster_rcnn_final_iter_320000.caffemodel')
