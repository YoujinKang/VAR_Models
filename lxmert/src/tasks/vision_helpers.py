...
class GroupedBatchSampler(BatchSampler):
    ...
    """
    def __init__(self, sampler, group_ids, batch_size, exhaustive = False):
    ...
    def __iter__(self):
        ...
        for idx in self.sampler:
            ...
            if len(buffer_per_group[group_id]) == self.batch_size:
                current = buffer_per_group[group_id]
                for i in current:
                    all_examples.add(i)
                yield current
                num_batches += 1
...
def create_aspect_ratio_groups_cache(dataset, k=0):
